...
- Do not use local functions. Extract behavior into private methods or dedicated classes instead.
- Always log SQL commands using the standard pattern:
  - `Debug`: compact SQL summary (single-line/truncated if needed).
  - `Trace`: full SQL text and SQL parameters.
...
- Avoid reading whole chunk datasets into memory. Prefer streaming end-to-end (source read -> transform/write -> target load) whenever possible.
